...
def convert_detection_to_panoptic_coco_format_single_core(
    ...
    for working_idx, img_id in enumerate(img_ids):
        ...
        if np.sum(overlaps_map > 1) != 0:
            print(np.sum(overlaps_map > 1))
            print(img)
            print("core", proc_id)
            print(overlaps_map[0])
            print(overlaps_map[1])
            print(overlaps_map)
            print(overlaps_map.shape)

            binary = np.asarray(overlaps_map, dtype="uint8")*100

            cv2.imwrite(img['file_name'], binary)
            #print(skr)
