    ...
    if args.input:
        ...
        for path in tqdm.tqdm(args.input, disable=not args.output):
            ...
            vis_frame = video_visualizer.draw_panoptic_seg_predictions(
                img, panoptic_seg.to('cpu'), segments_info
            )
            vis_frame = cv2.cvtColor(vis_frame.get_image(), cv2.COLOR_RGB2BGR)

            cv2.imwrite("yoyo.png", vis_frame)
            ...
            if args.output:
                ...
                #if cv2.waitKey(0) == 27:
                #    break  # esc to quit
                #visualized_output.save("yo.jpg")
                cv2.imwrite("yoNy2.jpg", visualized_output.get_image()[:, :, ::-1])
            '''
    elif args.webcam:
